...
# -*- coding: utf-8 -*-
"""
KUPA-KEYS Revision13: end-to-end reproduction of the adopted Model analysis.
...
--------
raw Task2 events
 -> text-state/cursor reconstruction (exact function from the accepted KUPA notebook)
...
----------------------
The historical internal name n_r_bursts_in_revisions is not used here.
The public/internal name is:

    n_backspace_sequences_with_gt1000ms_intervals

Publication label:
    Backspace sequences with >1,000-ms intervals

Operational definition:
    Number of physical consecutive Backspace sequences occurring within
    K-REV-defined revision episodes for which at least one Backspace press
    has an associated preceding key-down (press-to-press) interval > 1,000 ms.

The calculation is unchanged from the accepted implementation; only the
historical feature name has been replaced.
"""
...
# Exact set from the accepted KUPA reconstruction notebook.
NON_PRINTABLE_KEYS = {
...
def process_single_participant(user_df):
    ...
    for idx, row in user_df.iterrows():
        ...
        # 1. Universal Selection Bounds Tracker
        ...
        # 3. Primary Driver: Process physical keystrokes on 'down'
...
def sync_provenance_after_capture(old_text, old_origins, new_text):
    # Same conservative provenance synchronization used in the accepted extractor.
            ...
            if not (ctrl or meta):
                ...
                if key=="Backspace":
                    if lo==hi:
                        ...
                        # Accepted Base6 behavior: selection deletion has no
                        # conservatively observed single erased-character origin.
                        deleted_count=hi-lo
...
# Exact STRICT Pacquetet trace/sequence implementation from the accepted extractor.
def safe_float(x):
...
def build_strict_pacquetet_trace_and_sequences(user_df, pid):
    df = user_df.copy()
    df["_rank"] = event_rank(df["type"])
    df = df.sort_values(["time", "_rank"], kind="mergesort").reset_index(drop=True)
            ...
            if not (ctrl or meta):
                if key == "Backspace":
                    ...
                    if lo != hi:
                        ...
                        # A selection has multiple origins; no single exact Pacquetet TTE origin.
    ...
    while i < len(trace):
        ...
        # Reconstruct original deleted string. Consecutive Backspaces remove
        # right-to-left, so each deleted chunk is prepended.
        backspaced = ""
        ...
        # Pacquetet: replaced sequence = keys typed after the BS sequence,
        # corresponding to the number of backspaces. We only call it exact when
        # n_bs text-producing key-downs are observed before another Backspace.
        repl_chars = []
        ambiguous = False
        j = end + 1
...
def main():
    ...
    # 1. Reconstruction + validation + K-REV
    #
    # IMPORTANT: the accepted KUPA notebook did NOT send every reconstructed
    # Task-2 writing to K-REV. It first ran run_validations(), and participants
    # failing that reconstruction validation were removed from the validated
    # Task-2 results. K-REV inputs were then created only from that validated
    # dictionary. This validation step is therefore part of the accepted
    # analysis population and must be reproduced here.
    recon_all={}
    validation_rows=[]
    ...
    for pid,g in task2.groupby("id",sort=False):
        ...
        try:
            ...
            # Exact structural check from the accepted notebook.
            if len(text_list) != len(down_events):
                raise AssertionError(
                    f"text_states length ({len(text_list)}) != down events ({len(down_events)})"
                )

            # Exact cursor-bounds check from the accepted notebook.
            ...
            # Exact final-capture validation and tolerances from the accepted notebook.
    ...
    # Match notebook behavior: failed Task-2 reconstructions are deleted before
    # task2_results_validated_final.json is converted to K-REV inputs.
    ...
    # 3. Episode-level strict Conijn5 + writing-level Revision13.
    ...
    for pid,kd in recon.items():
        ...
        # Base6 source-faithful linkage: a physical sequence is covered when ANY
        # Backspace down-index from that sequence falls inside a KREV episode.
        # This linkage supplies ratio_gt3, >1000-ms count, and Time-to-Edit.
        base6_seq=(sq.loc[sq["backspace_sequence_id"].isin(sorted(covered))]
                   .drop_duplicates("backspace_sequence_id")
                   if not sq.empty else pd.DataFrame())
        nseq_base6=int(len(base6_seq))
        ratio_gt3=(float(base6_seq["is_gt3"].mean()) if nseq_base6 else np.nan)
        ngt1000=(int(base6_seq["gt1000ms_qualifies"].sum()) if nseq_base6 else 0)
        mean_tte=(float(pd.to_numeric(base6_seq["time_to_edit_ms"],errors="coerce").dropna().mean())
                  if nseq_base6 and pd.to_numeric(base6_seq["time_to_edit_ms"],errors="coerce").notna().any() else np.nan)

        # STRICT Pacquetet2 is intentionally computed from the STRICT extractor's
        # own trace/sequence implementation, not from the Base6 sequence table.
        # The accepted final model merged Base6 from the Base6 extractor and
        # Pacquetet2 from this separate STRICT source.
        raw_user = task2.loc[task2["id"].astype(str).eq(pid)].copy()
        _, strict_sq = build_strict_pacquetet_trace_and_sequences(raw_user, pid)

        # Exact STRICT linkage: sequence START must fall inside a KREV episode.
        # Writing-level count/mean use unique physical sequence IDs so nested or
        # overlapping KREV episodes cannot double-count the same sequence.
        strict_ids=set()
        if not strict_sq.empty:
            for r in eps.itertuples(index=False):
                lo,hi=sorted((int(r.index_start),int(r.index_end)))
                ids=(strict_sq.loc[
                    strict_sq["sequence_start_down_index"].between(lo,hi,inclusive="both"),
                    "backspace_sequence_id"
                ].astype(int).tolist())
                strict_ids.update(ids)

        strict_seq=(strict_sq.loc[
            strict_sq["backspace_sequence_id"].isin(sorted(strict_ids))
        ].drop_duplicates("backspace_sequence_id") if not strict_sq.empty else pd.DataFrame())

        nseq=int(len(strict_seq))
        mean_bs=(float(pd.to_numeric(
            strict_seq["pacquetet_n_backspaces_in_sequence"], errors="coerce"
    ...
    pd.DataFrame(conijn_episode).to_csv(OUT/"04_STRICT_Conijn_episode_features.csv",index=False)
    feat_all=pd.DataFrame(writing)
    feat_all.to_csv(OUT/"05a_Revision13_all_validated_writings.csv",index=False)

    # Exact accepted cross-task QC from the notebook:
    # Task-2 reconstruction validation retains 993 writings. Six additional
    # participants passed Task 2 but failed Task-1 validation; the accepted
    # analysis removes those six from Task 2 to preserve the matched QC sample.
    ...
    # 4. Composite CEFR target exactly as accepted final analysis.
    ...
    # Hard audit: stop rather than silently claim reproduction.
    # Accepted KUPA notebook reconstruction validation:
    # 1006 raw Task-2 writings -> 993 passed, 13 failed.
    if len(recon) != 993:
        raise RuntimeError(
    ...
    folds.to_csv(OUT/"09_Model4_10fold_CV_fold_results.csv",index=False)
    ...
    summary.to_csv(OUT/"10_Model4_10fold_CV_summary.csv",index=False)
    ...
    pd.DataFrame(cm,index=[f"true_{x}" for x in LEVELS],columns=[f"pred_{x}" for x in LEVELS]).to_csv(
        OUT/"12_Model4_OOF_confusion_matrix_counts.csv")
    pd.DataFrame(classification_report(oo["true_numeric"],oo["pred_numeric"],
        ...
        OUT/"13_Model4_OOF_classification_report.csv")
    ...
    coef.to_csv(OUT/"14_Model4_full_sample_standardized_coefficients.csv",index=False)
